...
export default function PinViewModal({ classes }) {
  const { user, pins, users } = useAuthState();
  const { id } = useParams();
  console.log(id);
  ...
  const [authorFollowCount, setAuthorFollowCount] = useState(false);
  ...
  useEffect(() => {
    ...
    if (pins != null) {
      ...
      const userFollowingRef = collection(db, 'public_users', `${pin_author}`, 'followers');
      onSnapshot(userFollowingRef, (querySnapshot) => {
        const user_followers = [];
        querySnapshot.docs.forEach((doc) => {
          user_followers.push(doc.data());
        });
        console.log(user_followers.filter((usrs) => usrs.type));

        if (user_followers.filter((usrs) => usrs.type).length <= 0) {
          setAuthorFollowCount(0);
        } else {
          setAuthorFollowCount(user_followers.length);
        }
      });
    }
                          ...
                          <div class='w-[80%] flex items-center'>
                            ...
                            <div class='flex flex-col'>
                              ...
                              <span class='font-regular text-[12px]'>
                                {users?.filter((usr) => usr?.uid === pin?.author).map((usr) => usr?.followers_count)} {authorFollowCount} Followers
                              </span>
                            </div>
                          </div>
                          {user?.uid !== pin?.author && (
