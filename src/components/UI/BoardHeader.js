...
const BoardHeader = ({ user, author, description, title }) => {
        ...
        <div class='flex justify-center'>
          {user?.photoURL === '' ? (
            ...
            <img
              ...
              src={user?.photoURL || users?.filter((usr) => usr.uid == author)?.photoURL}
            />
          )}
        </div>

        <div class='flex flex-col items-center'>
          <h1 class='text-[2.7rem] font-bold leading-4 leading-[1.5] mt-[1rem]'>{title}</h1>
          <h6 class='text-black mb-[1rem] font-[400] text-[14px]'>{description}</h6>
        </div>
